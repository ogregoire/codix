...
pub fn discover_files<'a>(
...
) -> Vec<(PathBuf, &'a dyn LanguagePlugin)> {
    ...
    for entry in WalkDir::new(root).into_iter().filter_map(|e| e.ok()) {
        let path = entry.path();
        if path.starts_with(root.join(".codix")) {
            continue;
        }
...
pub fn incremental_reindex(
    ...
    for (path, plugin) in &disk_files {
        ...
        match store.get_file(&rel_path)? {
            None => {
                // New file — index it
                let (file_id, wildcards): (FileId, Vec<String>) =
...
mod tests {
    ...
    fn setup_project() -> (TempDir, PathBuf) {
        ...
        fs::write(
            root.join("src/main/java/com/foo/Foo.java"),
            "package com.foo;\npublic class Foo {}",
        )
        .unwrap();
        fs::write(
            root.join("src/main/java/com/foo/Bar.java"),
            "package com.foo;\npublic class Bar {}",
        )
        .unwrap();
    ...
    fn test_incremental_reindex_detects_new_file() {
        ...
        fs::write(
            root.join("src/main/java/com/foo/Baz.java"),
            "package com.foo;\npublic class Baz {}",
        )
        .unwrap();
    ...
    fn test_wildcard_import_resolution() {
        ...
        fs::write(
            root.join("src/main/java/com/bar/Client.java"),
    ...
    fn test_ambiguous_wildcard_not_resolved() {
        ...
        fs::write(
            root.join("src/main/java/com/other/Foo.java"),
            "package com.other;\npublic class Foo {}",
        )
        .unwrap();
    ...
    fn test_incremental_reindex_detects_modified_file() {
        ...
        let syms = store
            .symbols_in_file("src/main/java/com/foo/Foo.java")
            .unwrap();
